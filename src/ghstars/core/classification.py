...
def _snapshot(stars: list[Star], lists: list[List]) -> str:
    payload = {
        "stars": [star.model_dump(mode="json") for star in stars if not star.archived],
        "lists": [lst.model_dump(mode="json") for lst in lists],
    }
...
def extract_work(stars: list[Star], lists: list[List], work_dir: Path) -> Manifest:
    ...
    manifest = Manifest(
        snapshot=_snapshot(active, classified_lists),
...
def render_markdown(work_dir: Path, output: Path, threshold: int) -> dict[str, int]:
    ...
    if len(proposals) != len(work.manifest.repos):
        missing = sorted(set(work.manifest.repos) - {item.repo for item in proposals})
        raise ClassificationError(f"missing proposals for: {', '.join(missing)}")
    rows: list[str] = [
        "| Repository | Current Lists | Target Classifications |",
        "| --- | --- | --- |",
    ]
    unclassified = 0
    for index, record in enumerate(proposals, 1):
...
def _load_proposals(
    ...
    try:
        ...
        for item in envelope:
            ...
            result.append(ProposalRecord.model_validate(payload))
        return result
    except (OSError, json.JSONDecodeError, ValidationError) as exc:
