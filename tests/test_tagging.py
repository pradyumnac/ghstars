...
def test_tag_star_creates_a_missing_list_for_real_and_pushes_immediately(
    ...
    assert created[0].items == ["pradyumnac/ghstars"]
...
def test_tag_star_accumulates_across_two_immediate_calls(
    ...
    tag_star(client, store, "pradyumnac/ghstars", "Explore: A")
...
def test_tag_star_is_idempotent_when_already_tagged(
    ...
    lst = List(id="L_1", name="Explore: A", slug="a", items=["pradyumnac/ghstars"])
    store = StateStore(tmp_path)
    store.save_stars([star])
    store.save_lists([lst])
    client = FakeGitHubClient(stars=[star], lists=[lst])
...
def test_tag_star_does_not_strip_across_different_categories(
    ...
    explore_a = List(
        id="L_a", name="Explore: A", slug="a", items=["pradyumnac/ghstars"]
    )
...
def test_tag_star_blocks_and_names_diverged_lists_when_local_is_stale(
    ...
    new_list = List(
        id="L_new", name="Explore: New", slug="new", items=["pradyumnac/ghstars"]
    )
    star = make_star("pradyumnac/ghstars", list_ids=["L_old"])
    store = StateStore(tmp_path)
    store.save_stars([star])
    ...
    with pytest.raises(StarListMembershipDriftError) as exc_info:
        tag_star(client, store, "pradyumnac/ghstars", "Explore: New")

    assert exc_info.value.full_name == "pradyumnac/ghstars"
    ...
    saved = {s.full_name: s for s in store.load_stars()}
    assert saved["pradyumnac/ghstars"].list_ids == ["L_old"]
    assert client.fetch_stars()[0].list_ids == ["L_old"]  # unchanged, no push
    assert "pradyumnac/ghstars" in client.fetch_lists()[1].items  # L_new unchanged too
...
def test_tag_star_fails_outright_with_no_local_write_on_push_failure(
    ...
    with pytest.raises(TagPushError) as exc_info:
        tag_star(client, store, "pradyumnac/ghstars", "Explore: Tool")

    assert exc_info.value.full_name == "pradyumnac/ghstars"
    saved = {s.full_name: s for s in store.load_stars()}
    assert saved["pradyumnac/ghstars"].list_ids == []
    assert saved["pradyumnac/ghstars"].pending_list_ids is None
...
def test_tag_star_pushes_via_a_pre_resolved_node_id_when_supplied(
    ...
    result = tag_star(
        ...
        "pradyumnac/ghstars",
        "Explore: Tool",
        node_id="pradyumnac/ghstars",  # FakeGitHubClient's node IDs == full_name
    )
